        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '32px' }}>
          ...
          <div style={{ background: '#13131a', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', overflow: 'hidden', ...fi(.3) }}>
            ...
            {onglet === 'retard' && enRetard.map((l, i) => {
              const j = joursEnRetard(l.date_retard), bc = j >= 20 ? '#f87171' : j >= 10 ? '#fb923c' : j >= 5 ? '#fbbf24' : '#94a3b8', bbg = j >= 20 ? 'rgba(248,113,113,.12)' : j >= 10 ? 'rgba(251,146,60,.12)' : j >= 5 ? 'rgba(251,191,36,.12)' : 'rgba(148,163,184,.1)', score = scoreRisque(l)
              return (
                <div key={l.id} className="ri ain" style={{ animationDelay: i * .04 + 's', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flex: 1, minWidth: 0 }}>
                    <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: 'rgba(248,113,113,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '15px', fontWeight: '700', color: '#f87171' }}>{l.nom.charAt(0).toUpperCase()}</div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                    <span className="bdg" style={{ background: bbg, color: bc }}>{j}j</span>
