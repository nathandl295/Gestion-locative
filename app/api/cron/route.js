...
function joursEnRetard(dateRetard) {
  ...
  return Math.floor((new Date() - new Date(dateRetard)) / (1000 * 60 * 60 * 24))
}
...
export async function GET(request) {
  ...
  // Récupérer le nom de l'agence pour l'expéditeur
  const { data: agence } = await supabase.from('agence').select('*').single()
  const nomAgence = agence?.nom || 'GestImmo'

  // 1. Repasser les locataires payés en attente si échéance dépassée
  const { data: payesAVerifier } = await supabase.from('locataires').select('*').eq('statut', 'paye')
  ...
  for (const locataire of payesAVerifier || []) {
    ...
    if (jourDuMois === echeance + 1) {
      await supabase.from('locataires').update({ statut: 'en_attente' }).eq('id', locataire.id)
      repassesEnAttente++
    }
  }

  // 2. Envoyer les relances automatiques
  const { data: locataires } = await supabase.from('locataires').select('*').eq('statut', 'en_retard')
  const { data: templates } = await supabase.from('templates').select('*').eq('auto_active', true).order('jours_min')
  ...
  for (const locataire of locataires || []) {
    ...
    if (locataire.derniere_relance_auto) {
      const diffJours = Math.floor((aujourd_hui - new Date(locataire.derniere_relance_auto)) / (1000 * 60 * 60 * 24))
    ...
    await resend.emails.send({
      from: `${nomAgence} <onboarding@resend.dev>`,
      to: locataire.email,
      subject: sujet,
      html: corps
    })
